...
public interface CardRepository extends GraphRepository<Card>, RelationshipOperationsRepository<Card> {
    ...
    @Query("MATCH (p:CardPack {name: {0}})<-[:POOL]-(:Tournament)<-[:IN_TOURNAMENT]-(:Standing {topdeck: true})-[:IS_DECK]->(:Deck)-[:HAS_CARD]->(c:Card {side_code: {1}})-[:IN_SET]->(cp:CardPack) " +
            "RETURN c.title AS title, COUNT(c) AS count, cp.name AS cardpack, c.faction_code AS faction ORDER BY count DESC LIMIT 20")
